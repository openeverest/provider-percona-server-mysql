...
CHART_DIR ?= charts/provider-percona-server-mysql
...
helm-install: helm-deps ## Install the provider using Helm.
	helm install provider-percona-server-mysql $(CHART_DIR) --create-namespace

.PHONY: helm-upgrade
helm-upgrade: helm-deps ## Upgrade the provider using Helm.
	helm upgrade provider-percona-server-mysql $(CHART_DIR)

.PHONY: helm-uninstall
helm-uninstall: ## Uninstall the provider using Helm.
	helm uninstall provider-percona-server-mysql

.PHONY: helm-template
helm-template: helm-deps ## Render Helm chart templates locally (dry-run).
	helm template provider-percona-server-mysql $(CHART_DIR)
